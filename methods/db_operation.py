...
def change_frequency():
    ...
    # datalist = sorted(return_data['data'], key=operator.itemgetter('change_times'), reverse = True)
    return return_data
    ...
    for index, each_visit_res in enumerate(item['ip_geo']): # 遍历每一次访问
        if index == 0:
            for ip in each_visit_res['ips']:
                ip_period[ip] = {'begin': item['record_time'], 'end': each_visit_res['insert_time']}
                last_ip.append(ip)
        else:
            ...
            for ip in each_visit_res['ips']:
                if ip in last_ip: # 上一次出现过，即连续出现的ip，更新最后插入时间作为计时结尾
                    ...
                    ip_period[ip] = {'begin': each_visit_res['insert_time'], 'end':''}
                temp.append(ip)
